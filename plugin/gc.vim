...
command GC ls! h | call feedkeys(':bd '
			...
			\->filter('empty(v:val.windows) && !v:val.changed')
			\->map('v:val.bufnr')
			\->join(' '))
...
function s:gc()
	...
	let garbage = getbufinfo({'buflisted': 1})
		\->filter({i,v -> empty(v.windows) && !v.changed
		...
		\->filter({i,v -> getbufvar(v.bufnr, '&buflisted')
			\ && i >= g:gc_delete_age - g:gc_unlist_age})
		\->map('v:val.bufnr')
		\->join(' ')
	if !empty(garbage)
